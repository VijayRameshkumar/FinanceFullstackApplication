...
def load_vessel_data(file_path: str) -> pd.DataFrame:
    """Load vessel data from an Excel file."""
    try:
...
def filter_vessel_data(vessel_df: pd.DataFrame, vessel_type: str, vessel_subtype: list, age_start: int, age_end: int) -> pd.DataFrame:
    """Filter vessel data based on type, subtype, and age."""
    vessel_df['VESSEL AGE'] = pd.to_datetime(vessel_df['DATE']).dt.year - vessel_df['BUILD YEAR']
    filtered_df = vessel_df[
        (vessel_df['VESSEL TYPE'] == vessel_type) & 
        (vessel_df['VESSEL SUBTYPE'].isin(vessel_subtype)) &
        (vessel_df['VESSEL AGE'].between(age_start, age_end))
    ]
    return filtered_df
